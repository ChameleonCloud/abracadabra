...
def test_nfs(uselease, keyname):
    session = ccmanageauth.session_from_args()
    
    if not uselease:
        # create a lease
        lease = ccmanagelease.Lease(session, name='appliance-test-nfs', length=datetime.timedelta(minutes=60), node_type='compute_haswell', nodes=3)
    else:
        # use existing lease
    ...
    with lease:
        ...
        stack = ccmanagestack.Stack(url='https://www.chameleoncloud.org/appliances/api/appliances/25/template', 
                                    verbose=True,
                                    exit_delay=60,
