...
IMAGE_TEST_MATRIX = {'compute_haswell': ['CC-CentOS7', 'CC-Ubuntu16.04', 'CC-Ubuntu18.04'],
                     ...
                     # two images at two different sites
                     'fpga': ['CC-CentOS7-FPGA'],
...
def reserve_resource_for_test(jenkins_location, booking_site, node_type, image_name):
    job_name = JOB_NAME_FORMAT.format(node_type=node_type.replace(
        '_', '-'), image_name=image_name.lower())
    if node_type == 'fpga':
        job_name = job_name + '-' + booking_site
    job_config_file = jenkins_location + '/' + \
        jenkinshelper.JENKINS_JOB_CONFIG_FILE.format(job_name=job_name)

    cctest_openrc_file = jenkins_location + '/' + \
        jenkinshelper.JENKINS_CCTEST_CREDENTIAL_FILE.format(site=booking_site)
    command = '\n'.join(['#!/bin/bash',
                         'source {cctest_openrc}'.format(
                             cctest_openrc=cctest_openrc_file),
                         'cd tests',
...
def reserve_resource_for_complex_appliance_test(jenkins_location, booking_site, complex_appliance_name, node_type, node_count):
    job_name = COMPLEX_JOB_NAME_FORMAT.format(
        complex_appliance_name=complex_appliance_name)
    job_config_file = jenkins_location + '/' + \
        jenkinshelper.JENKINS_JOB_CONFIG_FILE.format(job_name=job_name)
    ...
    if args.test_target == 'complex':
        ...
        if args.test_target in IMAGE_TEST_MATRIX:
            for image_name in IMAGE_TEST_MATRIX[node_type]:
                if node_type == 'fpga':
                    reserve_resource_for_test(
                        args.jenkins_jobs_location, 'uc', node_type, image_name)
                reserve_resource_for_test(
                    args.jenkins_jobs_location, 'tacc', node_type, image_name)
        else:
